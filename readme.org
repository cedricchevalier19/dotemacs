...
(use-package cmake-ide
  :after projectile
  :hook (c++-mode . cc/cmake-ide-find-project)
  :preface
  (defun cc/cmake-ide-find-project ()
  ...
  (defun cc/switch-to-compilation-window ()
    "Switches to the *compilation* buffer after compilation."
    (other-window 1))
  :bind ([remap comment-region] . cmake-ide-compile)
  :init (cmake-ide-setup)
  :config
  (advice-add 'cmake-ide-compile :after #'cc/switch-to-compilation-window)
...
#+RESULTS:
| python-black-on-save-mode-enable-dwim | yasnippet-snippets--fixed-indent | lsp-deferred | yapf-mode |
...
(use-package ivy
  ...
  :custom
    (ivy-re-builders-alist
     '((ivy-swith-buffer . ivy--regex-fuzzy)
...
(defun cc/org-roam-node-from-cite (keys-entries)
...
,#+filetags: %(kb/insert-lit-category)\n")
  ...
  :bind ("C-c c" . cc/org-roam-node-from-cite)
)
...
(use-package citar
  ...
  :init
  (setq citar-bibliography cc/bibfiles))
